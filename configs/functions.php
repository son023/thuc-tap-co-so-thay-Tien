...
function showRegister($list)
{
	...
	foreach ($list as $key => $li) {
		if (!is_null($key)) {
			...
			echo
				'<td>';
			foreach ($creditlist->getListSchedule()->getSchedule() as $key => $schedule) {
				if (!is_null($schedule)) {
...
function checkClassCredit($classCredit1,$classCredit2){
	...
	foreach($classCredit1->getListSchedule()->getSchedule() as $key1=>$li1){
	
		foreach($classCredit2->getListSchedule()->getSchedule() as $key2=>$li2){
